 ...
 * - https://example.com/dc-dojo-ladder/dist/ → dc-dojo-ladder
 */
...
describe('Trophy report filename', () => {
  ...
  describe('Subdirectory deployment', () => {
    it('should use first path segment', () => {
      expect(getTrophyFilename('/dc-dojo-ladder/dist/', 'example.com', dateStr)).toBe('dc-dojo-ladder-trophies_2026-05-28.tab');
    });

    it('should work without /dist/ suffix', () => {
      expect(getTrophyFilename('/dc-dojo-ladder/', 'example.com', dateStr)).toBe('dc-dojo-ladder-trophies_2026-05-28.tab');
    });
  ...
  describe('Production root deployment', () => {
    it('should use hostname when deployed to /dist/', () => {
      expect(getTrophyFilename('/dist/', 'example.com', dateStr)).toBe('example.com-trophies_2026-05-28.tab');
    });

    it('should use hostname when deployed to /', () => {
      expect(getTrophyFilename('/', 'example.com', dateStr)).toBe('example.com-trophies_2026-05-28.tab');
    });
  });

  describe('filename format', () => {
    it('should include date in ISO format', () => {
      expect(getTrophyFilename('/dc-dojo-ladder/dist/', 'example.com', '2026-01-15')).toBe('dc-dojo-ladder-trophies_2026-01-15.tab');
    });
