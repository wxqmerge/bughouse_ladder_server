...
export function parseEntry(
  ...
  playersList[3] = 0;
  playersList[4] = 0;
  // Note: playerOrRow is global in VB6, we'll omit it for now or move it to shared
...
/**
 * VB6 Line: 397-404 - String to long conversion (wrapper for parseEntry)
 */
...
/**
 * VB6 Line: 407-410 - Long to string conversion
...
/**
 * VB6 Line: 437-442 - Reset placement tracking
 */
...
/**
 * VB6 Line: 444-447 - Hash function initialization
...
export function processGameResults(
  ...
  if (shouldLog(10)) {
    ...
    for (let i = 0; i < localHashArray.length; i++) {
      if (localHashArray[i] !== "" && localHashIndex[i] !== 0) {
        hashResults.push({
          index: i,
          keyVal: localHashIndex[i],
          value: localHashArray[i],
        });
        console.log(
          `Hash[${i}]: keyVal=${localHashIndex[i]}, value="${localHashArray[i]}"`,
        );
      }
    }
    if (shouldLog(5)) {
...
export function repopulateGameResults(
  ...
  for (const match of matches) {
    // Build the normalized result string (same for all players in this match)
    const normalizedResult = buildNormalizedResult(match);

    if (shouldLog(5)) {
      console.log(
        `[REPOPULATE] Match: p1=${match.player1}, p2=${match.player2} -> "${normalizedResult}"`,
      );
    }

    // Set result for each player in the match
    ...
    for (const playerRank of playerRanks) {
      ...
      if (round >= 0 && normalizedResult) {
        ...
        if (shouldLog(5)) {
          console.log(
            `  -> Set player ${playerRank}[${round}] = "${normalizedResult}_"`,
          );
        }
      } else if (round < 0) {
        if (shouldLog(5)) {
          console.log(`  -> SKIPPED player ${playerRank} (no empty rounds)`);
        }
      }
    }
  }
