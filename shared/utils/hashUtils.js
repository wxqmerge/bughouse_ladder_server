 ...
 */
export function entry2string(playersList, scoreList) {
 ...
 */
export function parseEntry(myText, playersList, scoreList) {
 ...
 */
export function string2long(game, playersList, scoreList) {
 ...
 */
export function long2string(game) {
    const resultParts = [];
    let tempGame = game;
 ...
 */
export function hashInitialize() {
...
export let hashArray = [];
export let hashIndex = [];
/**
...
export function processGameResults(playersList, numRounds = 31) {
    ...
    const swapScore = (code) => {
        if (code === 0)
            return 0; // O stays O
        if (code === 1)
            return 3; // L becomes W
        if (code === 2)
            return 2; // D stays D
        if (code === 3)
            return 1; // W becomes L
        return 0;
    };
    ...
    const normalizeResultForComparison = (_result, _playerRank, parsedPlayers, scores) => {
        ...
        if (!is4Player) {
            ...
            const scoreLetter = score === 0 ? "O" : score === 1 ? "L" : score === 2 ? "D" : "W";
            ...
            const score1Letter = normScore1 === 0
                ? "O"
                : normScore1 === 1
                    ? "L"
                    : normScore1 === 2
                        ? "D"
                        : "W";
            const score2Letter = normScore2 === 0
                ? "O"
                : normScore2 === 1
                    ? "L"
                    : normScore2 === 2
 ...
 */
export function calculateRatings(playersList, matches, _kFactorOverride) {
    let kFactor = 20;
    ...
    for (const match of matches) {
        ...
        const p1NewRating = Math.round(p1Rating + EloKfactor * (actualP1 - expectedP1));
        const p2NewRating = Math.round(p2Rating + EloKfactor * (actualP2 - expectedP2));
