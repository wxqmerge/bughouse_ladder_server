 ...
 */
export const RESULT_STRING = "OLDWXYZ__________";
/**
 * VB6 Line: 61 - Group codes for player classification
 */
export const GROUP_CODES = "A1xAxBxCxDxExFxGxHxIxZx   ";
/**
 ...
 */
function scoreCodeToLetter(code) {
 ...
 */
export function normalize4Player(a1, a2, a3, a4) {
 ...
 * e.g., 13,12 → 12,13
 */
export function normalize2Player(a, b) {
    return a < b ? [a, b] : [b, a];
}
/**
 ...
 */
export function formula(myRating, opponentsRating) {
 ...
 */
function entry2string(playersList, scoreList) {
 ...
 */
function parseEntry(myText, playersList, scoreList) {
    ...
    if (playersList[2] > 0) {
        ...
        if (playersList[0] > playersList[2]) {
            ...
            if (scoreList[1] > 0)
                scoreList[1] = 4 - scoreList[1];
        }
        // VB6 Line: 284-292 - Create hash value
        const res = playersList[3];
        const computedRes = ((((res * 128 + playersList[2]) * 4 + scoreList[1]) * 4 + scoreList[0]) *
            128 +
            playersList[1]) *
            128 +
            playersList[0];
        ...
        const computedRes = ((playersList[0] * 128 + playersList[1]) * 4 + scoreList[0]) * 128 + 0;
        return computedRes;
    }
}
/**
 ...
 */
function string2long(game, playersList, scoreList) {
 ...
 */
export function processGameResults(playersList, numRounds = 31) {
    const results = [];
    const errors = [];
    ...
    let localHashArray = new Array(2048).fill("");
    let localHashIndex = new Array(2048).fill(0);
    // Helper to perform hash operations using local state
    const dataHashLocal = (skey, sval, hashMethod) => {
    ...
    const hashInitializeLocal = () => {
        const rand8 = Array.from({ length: 256 }, (_, i) => i);
        let k = 7;
        for (let j = 0; j < 3; j++) {
            for (let i = 0; i < 256; i++) {
                const s = rand8[i];
    ...
    const normalizeResultForComparison = (_result, _playerRank, parsedPlayers, scores) => {
        ...
        if (!is4Player) {
            ...
            let normPair1, normPair2, normScore1, normScore2;
    ...
    for (let round = 0; round < numRounds; round++) {
        const processedPairs = new Set(); // Use string keys for normalized deduplication within round
    ...
    for (const [_, entries] of matchResults.entries()) {
        ...
        const normalizedEntries = entries.map((e) => {
            ...
            return normalizeResultForComparison(e.result, e.playerRank, parsedPlayers, scores);
        });
        const allSame = normalizedEntries.every((n, i) => i === 0 || n === normalizedEntries[0]);
        if (!allSame) {
 ...
 */
function calculateRatingsSinglePass(playersList, matches, EloKfactor, debugMode, passLabel, blendingFactor = 1, perfMultiplierScale = 1) {
    ...
    dbg.group(`${label}[INIT] VB6 lines 1422-1449 — k_val = ${EloKfactor}`, () => {
        for (const p of playersCopy) {
            ...
            dbg.log(`P${p.rank}: num_games=${p.num_games}, rating=${p.rating}, nRating=${p.nRating} → nrating=${nrating}`);
        }
    });
    ...
    for (const match of matches) {
        ...
        const scores = [match.score1, is4Player ? match.score2 : 0];
        const matchTrace = {
        ...
        dbg.group(`${label}[MATCH] VB6 lines 1474-1535 — ${is4Player ? "4-player" : "2-player"}`, () => {
            ...
            dbg.log(`FIRST LOOP (W/L/D): perfs=(${wldPerfs0.toFixed(2)}, ${wldPerfs1.toFixed(2)})`);
            ...
            dbg.log(`PERF RATING (side-level, for 2p/trace): sides=(${matchTrace.perfRatings[0].toFixed(1)}, ${matchTrace.perfRatings[1].toFixed(1)})`);
            ...
            dbg.log(`SECOND LOOP (expected diff): eloPerfs=(${eloPerfs0.toFixed(4)}, ${eloPerfs1.toFixed(4)})`);
            ...
            for (const player of side0Players) {
                ...
                if (games > 9) {
                    ...
                    dbg.log(`  P${player.rank} [side 0, ${games} games]: ELO → ${nratingBefore.toFixed(1)} + ${eloPerfs0.toFixed(4)} × ${EloKfactor} = ${raw.toFixed(1)} → abs = ${Math.abs(raw).toFixed(1)}`);
                }
                else {
                    ...
                    dbg.log(`  P${player.rank} [side 0, ${games} games]: BLEND → (${nratingBefore.toFixed(1)} × ${games} + ${playerPerfRating.toFixed(1)}) / ${games + 1} = ${blended.toFixed(1)} → abs = ${Math.abs(blended).toFixed(1)}`);
                }
            ...
            for (const player of side1Players) {
                ...
                if (games > 9) {
                    ...
                    update.nRatingAfter = Math.abs(raw);
                    currentRating.set(player.rank, Math.abs(raw));
                    dbg.log(`  P${player.rank} [side 1, ${games} games]: ELO → ${nratingBefore.toFixed(1)} + ${eloPerfs1.toFixed(4)} × ${EloKfactor} = ${raw.toFixed(1)} → abs = ${Math.abs(raw).toFixed(1)}`);
                }
                else {
                    update.formula = "blend";
                    update.opposingPerfRating = playerPerfRating;
                    ...
                    update.nRatingAfter = Math.abs(blended);
                    currentRating.set(player.rank, Math.abs(blended));
                    dbg.log(`  P${player.rank} [side 1, ${games} games]: BLEND → (${nratingBefore.toFixed(1)} × ${games} + ${playerPerfRating.toFixed(1)}) / ${games + 1} = ${blended.toFixed(1)} → abs = ${Math.abs(blended).toFixed(1)}`);
                }
                matchTrace.playerUpdates.push(update);
                careerGames.set(player.rank, games + 1);
            }
 ...
 */
export function calculateRatings(playersList, matches, options) {
    ...
    // === PASS 1 ===
    const pass1 = calculateRatingsSinglePass(playersList, matches, kFactor, debugMode, "[PASS 1] ", blendingFactor, perfMultiplierScale);
    const pass1NRating = new Map();
    ...
    const pass2 = calculateRatingsSinglePass(pass1.players, matches, kFactor, debugMode, "[PASS 2] ", blendingFactor, perfMultiplierScale);
    const pass2NRating = new Map();
 ...
 */
export function repopulateGameResults(playersList, matches, numRounds = 31, _playerResultsByMatch) {
    const playersCopy = playersList.map((p) => ({
        ...p,
        gameResults: new Array(numRounds).fill(null),
    }));
    const findLowestEmptyRound = (player) => {
    ...
    const seen2pOpponents = new Map();
    const seen4pOpposingPairs = new Map();
    const ensure2pSet = (rank) => {
        if (!seen2pOpponents.has(rank))
            seen2pOpponents.set(rank, new Set());
        return seen2pOpponents.get(rank);
    };
    const ensure4pSet = (rank) => {
        if (!seen4pOpposingPairs.has(rank))
            seen4pOpposingPairs.set(rank, new Set());
    ...
    for (const match of matches) {
        ...
        if (!is4p) {
            ...
            if (s1.has(oppPair) || s2.has(oppPair) || s3.has(allyPair) || s4.has(allyPair))
                continue;
            s1.add(oppPair);
            s2.add(oppPair);
            s3.add(allyPair);
...
export function updatePlayerGameData(input, addUnderscore = true) {
    if (!input.trim()) {
        return {
            isValid: false,
            error: 3,
