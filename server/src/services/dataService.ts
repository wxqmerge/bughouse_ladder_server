...
try {
  const _pkg = JSON.parse(fsSync.readFileSync(path.join(__dirname, '../../package.json'), 'utf-8'));
  if (_pkg && typeof _pkg === 'object' && typeof _pkg.version === 'string') {
    _serverVersion = _pkg.version;
  }
} catch {
